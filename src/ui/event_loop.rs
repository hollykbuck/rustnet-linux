    ...
    <B as ratatui::prelude::Backend>::Error: Send + Sync + 'static,
{
    ...
    loop {
        ...
        if needs_data_refresh || last_tick.elapsed() >= tick_rate {
            ...
            devices = if ui_state.filter_query.is_empty() && !ui_state.filter_mode {
                app.get_devices()
            } else {
                app.get_filtered_devices(&ui_state.filter_query)
            };

            crate::app::state::sort_devices(
                &mut devices,
                ui_state.device_sort_column,
                ui_state.sort_ascending,
            );
