...
fn draw_top_processes(f: &mut Frame, connections: &[Connection], area: Rect) {
    ...
    let mut process_traffic: std::collections::HashMap<String, f64> =
    ...
    let rows: Vec<Row> = sorted
        ...
        .map(|(name, rate)| {
            ...
            Row::new(vec![
                ...
                Cell::from(Line::from(format_rate(rate)).right_aligned()).style(fg(accent())),
            ])
        })
        .collect();
    ...
    if rows.is_empty() {
        f.render_widget(
            Paragraph::new("No active processes").style(fg(muted())),
            inner,
        );
        return;
    }
...
fn draw_health_chart(f: &mut Frame, history: &TrafficHistory, area: Rect) {
    ...
    if !history.has_enough_data() {
        f.render_widget(
            Paragraph::new("Collecting data...").style(fg(muted())),
            inner,
        );
        return;
    }
    ...
    let rtt_line = if let Some(rtt) = current_rtt {
        ...
        Line::from(vec![
            ...
            Span::styled("░".repeat(empty), fg(muted())),
            Span::styled(format!(" {:>6.1}ms", rtt), fg(color)),
        ])
    } else {
        Line::from(vec![
            Span::styled("  RTT  ", Style::default().add_modifier(Modifier::BOLD)),
            Span::styled("░".repeat(bar_width), fg(muted())),
    ...
    let loss_line = Line::from(vec![
        ...
        Span::styled("░".repeat(empty.min(bar_width)), fg(muted())),
        Span::styled(format!(" {:>6.2}%", current_loss), fg(loss_color)),
    ]);

    let avg_line = Line::from(vec![
        Span::styled("  avg: ", fg(muted())),
        Span::styled(
            avg_rtt
                .map(|r| format!("{:.0}ms", r))
        ...
        Span::styled(" / ", fg(muted())),
        Span::styled(format!("{:.2}%", avg_loss), fg(muted())),
    ]);
...
fn draw_tcp_counters(f: &mut Frame, app: &App, area: Rect) {
    ...
    let lines = vec![
        Line::from(vec![
            ...
            Span::styled(format!("{:>8}", retransmits), fg(retrans_color)),
        ]),
        Line::from(vec![
            Span::styled(
                "  Out of Order ",
            ...
            Span::styled(format!("{:>8}", out_of_order), fg(ooo_color)),
        ]),
        Line::from(vec![
            Span::styled(
                "  Fast Retrans ",
            ...
            Span::styled(format!("{:>8}", fast_retransmits), fg(fast_color)),
        ]),
    ];
    f.render_widget(Paragraph::new(lines), inner);
}
...
fn draw_tcp_states(f: &mut Frame, connections: &[Connection], area: Rect) {
    ...
    if states.is_empty() {
        f.render_widget(
            Paragraph::new("No TCP connections").style(fg(muted())),
            inner,
        );
        return;
    }
    ...
    let lines: Vec<Line> = states
        ...
        .map(|(name, count)| {
            ...
            Line::from(vec![
                Span::styled(format!("{:>10} ", name), fg(color)),
                Span::styled(bar, fg(color)),
